...
$(document).ready(function() {
	...
	$('#plusbtn').click(function() {
		...
		// 페이지 로딩 될때 설정했던 콤마 제거
		oneprice = removecomma(oneprice); 
		...
		if (person < max_person) {
			// 사용자가 선택하는 인원수가 사업자가 설정한 인원수 미만이면 + 1 처리 
			// 인원수 증가
			person_result = person + 1;

			// 파라미터로 넘어갈 총 금액
			// 실제 DB로 넘어갈 결제 총 금액
			totalprice = oneprice * person_result; 
			totalprice = addcomma(totalprice);

			// 사용자에게 보여줄 가짜 총 금액
			// 가짜 총 금액(사용자에게 보여줄 용도)
			fake_totalprice = oneprice * person_result; 
			fake_totalprice = addcomma(fake_totalprice);

			$('button#plusbtn').removeAttr('data-toggle', 'modal');
		} else {
			//사용자가 선택하는 인원수가 사업자가 설정한 인원수 초과이면 + 0 처리 
