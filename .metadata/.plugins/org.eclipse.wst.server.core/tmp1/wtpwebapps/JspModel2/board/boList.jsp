...
<head>
	...
	<script type="text/javascript">/* 12/10 복사 부분 */
		function writeForm(){// 글쓰기 , 위치 바꿔주는 locaion 사용 
				location.href='<%=NoForm%>boInsert';
		}
		function search(){ // val() : value 속성 읽기
			if( $('#mode').val() == 'all' ){
				alert('검색 목록을 선택해주세요') ;
				//$('#mode').focus();
			}else{
		...
		function searchAll(){//boList 호출을 하면 BoardListController 로가고 파라미터가 없기때문에 전체를 다시 돈다. 
			//$('#mode').val('-');
			//$('#keyword').val('');
...
<body>
	<div class="container col-sm-offset-<%=myoffset%> col-sm-<%=mywidth%>">
		...
		<div align="center"> <!-- 12/10 복사 부분 -->
			<footer>${requestScope.pagingHtml}</footer>			
		</div>	<!-- 12/10 복사 부분 -->	
	</div>
	<br><br><br><br><!-- 12/10 복사 부분 -->
